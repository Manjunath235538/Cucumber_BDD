...
public class ActionsHelper {
	// Declarations
		...
		/**
		 * Constructor for initializing the JavaScriptExecutor, FluentWait and assigning
		 * driver, logger instances for method interactions
		 * 
		 * @param driver
		 * @param logger
		 */
		public ActionsHelper(WebDriver driver, Logger logger) {
			this.driver = driver;
			this.logger = logger;
			jse = (JavascriptExecutor) driver;
			wait = new FluentWait<WebDriver>(driver).withTimeout(3, TimeUnit.MINUTES).pollingEvery(10, TimeUnit.SECONDS)
		...
		public void logScreenshot() {
			...
			File destin = new File("Screenshots" + File.separator + "Swiggy_"
		...
		/**
		 * Below method takes the screenshot and logs into the report with specified
		 * name
		 */
		public void logScreenshot(String screenshotName) {
			File source = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
			File destin = new File("Screenshots" + File.separator + screenshotName + "_"
					+ new SimpleDateFormat("ddMMyyyy_hhmmss").format(new Date()) + ".png");
			try {
				com.google.common.io.Files.copy(source.getAbsoluteFile(), destin);
				Reporter.addScreenCaptureFromPath(destin.getAbsolutePath());

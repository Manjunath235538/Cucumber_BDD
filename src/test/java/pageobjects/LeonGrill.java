...
public class LeonGrill implements Restaurant {
	...
	// Declarations
		 ...
		 * @param helper
		 */
		public LeonGrill(WebDriver driver, Logger logger, ActionsHelper helper) {
		...
		private WebElement leonGrill;

		@FindBy(xpath = "//div[h2[text()='Beverage']]//div[@itemprop]")
		private List<WebElement> allBeverages;

		@FindBy(xpath = "//div[h2[text()='Beverage']]//div[text()='ADD']")
		private List<WebElement> addBeverages;

		@FindBy(xpath = "//div[h2[text()='Beverage']]//div[text()='ADD']/following::div[text()='+']")
...
//		private WebElement customizePopUpAddItem;
		...
		public void verifyRestaurantTitle(String expectedTitle) {
			helper.waitForElementVisibility(this.leonGrill, 3);
			helper.isDisplayed(this.leonGrill);

			helper.validateActualAndExpected(this.leonGrill.getText().trim(), expectedTitle);
